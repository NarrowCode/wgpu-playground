...
// Indices stores the mapping of vertices so we don't have to repeat
// defining shared points multiple times
...
impl State {
    ...
    fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
        ...
        // The extra {} block borrows the encoder mutably (like &mut self)
        {
            ...
            // Since we're using an index buffer, we need to use `draw_indexed`
            render_pass.set_index_buffer(self.index_buffer.slice(..), wgpu::IndexFormat::Uint16);
            render_pass.draw_indexed(0..self.num_indices, 0, 0..1); // Draw with 3 vertices in 1 instance
